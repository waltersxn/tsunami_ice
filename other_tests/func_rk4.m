function [T,Y] = func_rk4(f,a,b,dt,Y0,c)
...
for j = 1:N
	K1 = feval(f, T(j),Y(:,j),c);
	K2 = feval(f, T(j) + 0.5*dt, Y(:,j) + 0.5*dt*K1,c);
	K3 = feval(f, T(j) + 0.5*dt, Y(:,j) + 0.5*dt*K2,c);
	K4 = feval(f, T(j) + dt, Y(:,j) + dt*K3,c);
